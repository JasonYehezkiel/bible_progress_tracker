...
class CRFBibleReferenceExtractor:
    ...
    def extract_ner_spans(self, text: str) -> List[Dict[str, Any]]:
        """
        Extract NER spans from text using the CRF model (token-level predictions).
    ...
    def extract_structure(self, text: str) -> List[Dict[str, Any]]:
        """
        Parse structured references from CRF-extracted NER spans.

        Args:
            text: Input text
        
        Returns:
            List of parsed reference dicts with keys
        """
