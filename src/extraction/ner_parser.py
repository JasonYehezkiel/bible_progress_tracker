...
logger = logging.getLogger('bible_pipeline.extraction.ner_parser')
...
TOKEN_RE = re.compile(
    r"(?P<RANGE>[—–]|-|sampai|sampe|smpe|hingga|s\s*/\s*d|s\.d\.|sd)"
    r"|(?P<COLON>:)"
    r"|(?P<SEMI>;)"
    r"|(?P<COMMA>,)"
...
def read_book(tokens: List[Token], pos: int) -> Optional[Tuple[str, int]]:
    ...
    if (idx + 1 < n
        ...
        if last_word == next_word or last_word in compound_roots or next_word in compound_roots:
            ...
            idx += 2
            logger.debug("Compound book name detected: %r", parts[-1])
            
    return " ".join(parts), idx

def consume_verse(tokens: List[Token], pos: int) -> int:
    """Consume a bare verse suffix (COLON NUM). Returns new pos."""
    n = len(tokens)
...
def parse_segment(
    ...
    """
    Parse a token segment starting at pos.
    
    Handles five patterns in priority order:
    - Numeric cross-book range
    - Standard
    - Context continuation
    - Cross-book range
    - Bare cross-book range

    Returns:
        Tuple of list of parsed reference dicts, updated token pos,
        and updated context book
    """
    n = len(tokens)
    ...
    if bm:
        ...
        logger.warning("No book token and no context book at pos %d; skipping token %r", pos, tokens[pos])
    ...
    if pos < n and tokens[pos][0] == "NUM":
        ...
        if pos < n and tokens[pos][0] == "COLON":
            ...
            if pos < n and tokens[pos][0] == "RANGE":
                peek = pos + 1
                if peek < n and tokens[peek][0] == "NUM":
                    peek2 = peek + 1
                    if peek2 < n and tokens[peek2][0] == "COLON":
                        end_chapter = int(tokens[peek][1])
                        pos = peek + 1
                        pos = consume_verse(tokens, pos)
                    else:
                        pos = peek + 1
...
def parse_ner_response(
        spans: List[str],
        initial_ctx_book: Optional[str] = None
) -> List[Dict]:
    """
    Parse a BIBLE_REF NER extraction string into structured reference dicts.

    Args:
        ner_text: NER output string
        initial_ctx_book: Optional intial context book for parsing.
 
    Returns:
        List of build_reference() dicts. Empty list if input is unrecognised
    """
    refs = []
    ctx_book = initial_ctx_book

    for span_text in spans:
        logger.debug("Parsing NER text: %r (ctx_book=%r)", span_text, ctx_book)
        tokens = tokenize(span_text)
        logger.debug("Tokens: %s", tokens)
        pos = 0
        while pos < len(tokens):
            if tokens[pos][0] in ("SEMI", "COLON", "COMMA"):
                pos += 1
                continue
