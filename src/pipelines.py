...
from services import apply_gap_fill, format_header
from config.settings import CONFIG_PATH, AGGREGATION_STRATEGY
from extraction.extractor import BibleReferenceExtractor
from reading_plan.checker import ComplianceChecker
from reading_plan.schedule import ReadingPlanSchedule
from sessions.database import get_session
from sessions.crud import (
...
class BibleProgressPipeline:
    ...
    def process(self, row: pd.Series) -> Dict:
        ...
        refs = self.extractor.extract(row['message'])
    ...
    def process_batch(self, df: pd.DataFrame) -> Dict:
        ...
        messages = [row['message'] for row in pending_rows]
