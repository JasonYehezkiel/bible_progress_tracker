...
class TestMergeSubwordSpans:
    def span(self, word, start=0, end=None):
        return {'entity_group': 'BIBLE_REF', 'word': word, 'score': 0.99,
                'start': start, 'end': end or len(word)}
    
    def test_no_subwords_returns_unchanged(self, extractor):
        result = extractor.merge_subword_spans([ner_hit('Kej 1'), ner_hit('Kej 2')])
        assert len(result) == 2
    
    def test_single_subword_merged_into_previous(self, extractor):
        result = extractor.merge_subword_spans([ner_hit('Ke'), ner_hit('##j')])
        assert len(result) == 1
        assert result[0]['word'] == 'Kej'
    
    def test_end_offset_updated_after_merge(self, extractor):
        sub = ner_hit('##j')
        result = extractor.merge_subword_spans([ner_hit('Ke'), sub])
        assert result[0]['end'] == sub['end']
    
    def test_multiple_consecutive_subword_merged(self, extractor):
        spans = [ner_hit('Ke'), ner_hit('##ja'), ner_hit('##dian')]
        result = extractor.merge_subword_spans(spans)
        assert len(result) == 1
        assert result[0]['word'] == 'Kejadian'
    
    def test_subword_with_no_prior_span_kept_as_is(self, extractor):
        result = extractor.merge_subword_spans([ner_hit('##j')])
        assert len(result) == 1
        assert result[0]['word'] == '##j'
    
    def test_non_subword_after_subword_starts_new_span(self, extractor):
        result = extractor.merge_subword_spans([ner_hit('Ke'), ner_hit('##j 1'), ner_hit('Ke'), ner_hit('##j 2')])
        assert len(result) == 2
        assert result[0]['word'] == 'Kej'
        assert result[1]['word'] == '1'
    
    def test_other_fields_preserved_after_merge(self, extractor):
        result = extractor.merge_subword_spans([ner_hit('Ke'), ner_hit('##j')])
        assert result[0]['entity_group'] == 'BIBLE_REF'
        assert result[0]['score'] == 0.99
    
    def test_non_subword_after_subword_starts_new_span(self, extractor):
        assert extractor.merge_subword_spans([]) == []
...
class TestBibleReferenceExtractor:
    ...
    def test_unparseable_span_logs_warning_and_returns_empty(self, extractor, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger='bible_pipeline.indobert.extractor'):
